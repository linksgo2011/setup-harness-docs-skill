---
name: qa-write-e2e
description: 编写和运行自动化测试 — 后端 API 集成测试（RestAssured）和前端 E2E 测试（Playwright）。适用于编写或运行测试的场景。
---
...
`backend/src/test/java/com/consultation/adapter/rest/` 中的测试类：
- `AuthControllerTest` — 注册、登录、重复邮箱、错误密码
- `AppointmentControllerTest` — 创建、列表、冲突、取消
- `AdminControllerTest` — 仪表盘、用户、分支机构 CRUD、数据字典、业务规则
...
`e2e/tests/app.spec.ts` 中的测试覆盖：公共页面、认证、用户生命周期、管理员流程、边界情况。

## 编写新测试

1. 在 `e2e/tests/app.spec.ts` 的现有 describe 块中添加测试，或创建新块
2. 遵循现有模式（先登录、交互、断言）
