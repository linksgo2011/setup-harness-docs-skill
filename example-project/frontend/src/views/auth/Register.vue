  ...
  <div class="auth-page">
    <div class="card auth-card">
      ...
      <form @submit.prevent="handleRegister">
        <div class="form-group"><label>姓名</label><input v-model="name" class="input" required placeholder="请输入姓名" /></div>
        <div class="form-group"><label>邮箱</label><input v-model="email" type="email" class="input" required placeholder="请输入邮箱" /></div>
        <div class="form-group"><label>手机号</label><input v-model="phone" class="input" placeholder="选填" /></div>
        <div class="form-group"><label>密码（至少8位）</label><input v-model="password" type="password" class="input" required minlength="8" placeholder="请输入密码" /></div>
...
async function handleRegister() {
  ...
  try { await auth.register(email.value, password.value, name.value, phone.value); router.push('/user/dashboard') }
  catch (e: any) { error.value = e.response?.data?.errorMessage || '注册失败' }
